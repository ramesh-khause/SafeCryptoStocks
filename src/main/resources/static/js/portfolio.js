document.addEventListener('DOMContentLoaded', async () => {
	...
	async function handleSellStock(event) {
	    ...
	    const sellQuantity = prompt('Enter the quantity of stock to sell:');
	    ...
	    try {
	        ...
	        if (response.ok) {
	            ...
	            fetchStocksByPortfolio(portfolioId);
	        } else {
	            const errorMessage = await response.text();
	            alert(`Failed to sell stock: ${errorMessage}`);
	        }
	    } catch (error) {
	        console.error('Error selling stock:', error);
	        alert('An error occurred while processing the sale. Please try again later.');
	    }
	}
